...
async def bot_control(session: CommandSession):
    suid = session.current_arg_text.strip()
    if not suid:
        suid = (await session.aget(prompt='请发送审批UID。')).strip()
    if suid in approve_dict:
        approve_dict[suid] = True
...
async def msg_relay(session: CommandSession):
    ...
    if mode == 1:
        ...
        if uid not in get_config("blacklist", "friend"):
            ...
            type = session.ctx['sub_type']
            msg = session.ctx['message']
            relay_header = f"收到来自QQ：{uid} 的私聊消息，消息类型：{type}。\n以下为消息内容："
